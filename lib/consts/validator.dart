class MyValidators {
  static String? displayNamevalidator(String? displayName) {
    if (displayName == null || displayName.isEmpty) {
      return 'Display name cannot be empty';
    }
    if (displayName.length < 3 || displayName.length > 20) {
      return 'Display name must be between 3 and 20 characters';
    }

    return null; // Return null if display name is valid
  }

  static String? emailValidator(String? value) {
    if (value!.isEmpty) {
      return 'Please enter an email';
    }
    if (!RegExp(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b')
        .hasMatch(value)) {
      return 'Please enter a valid email';
    }
    return null;
  }

  static String? passwordValidator(String? value) {
    if (value!.isEmpty) {
      return 'Please enter a password';
    }
    if (value.length < 6) {
      return 'Password must be at least 6 characters long';
    }
    return null;
  }

  static String? repeatPasswordValidator({String? value, String? password}) {
    if (value != password) {
      return 'Passwords do not match';
    }
    return null;
  }

  static String? uploadProdTexts({String? value, String? toBeReturnedString}) {
    if (value!.isEmpty) {
      return toBeReturnedString;
    }
    return null;
  }

  static String? imageUrlValidator(String? value) {
  if (value == null || value.isEmpty) {
    return 'Please enter an image URL';
  }
  // ตรวจสอบว่า URL มีความถูกต้อง
  bool isValidUrl = Uri.tryParse(value)?.hasAbsolutePath ?? false;
  if (!isValidUrl) {
    return 'Please enter a valid image URL';
  }
  // สามารถเพิ่มการตรวจสอบว่าเป็น URL ของรูปภาพ เช่น การตรวจสอบว่า URL ลงท้ายด้วย .jpg, .png เป็นต้น
  if (!value.endsWith('.jpg') && !value.endsWith('.png')) {
    return 'Please enter a valid image URL with .jpg or .png extension';
  }
  return null;
}
}
